  ...
  return (
    <div className="min-h-screen bg-[var(--cream)] flex items-center justify-center px-6">
      <motion.div className="w-full max-w-[400px]" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.23,1,0.32,1] }}>
        ...
        <div className="card p-8" style={{ boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
          ...
          {step === 'email' ? (
            <form onSubmit={sendOtp} className="space-y-5">
              <div><label className="text-[13px] font-semibold text-[var(--ink)] mb-2 block">Email</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="input-warm !rounded-xl" placeholder="you@company.com" required autoFocus /></div>
            ...
            <form onSubmit={verifyOtp} className="space-y-5">
              <div><label className="text-[13px] font-semibold text-[var(--ink)] mb-2 block">Code</label>
                <input type="text" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g,'').slice(0,6))} className="input-warm !rounded-xl text-center text-[28px] tracking-[0.4em] font-[var(--font-mono)] font-bold" placeholder="······" maxLength={6} required autoFocus /></div>
              ...
              <button type="button" onClick={() => { setStep('email'); setOtp(''); setError(''); }} className="w-full text-[13px] text-[var(--blue)] font-medium hover:underline py-1 bg-transparent border-none cursor-pointer">Different email</button>
            </form>
          )}
        </div>
      </motion.div>
