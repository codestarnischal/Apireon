...
function Cp({ text }: { text: string }) {
  ...
  return <motion.button onClick={() => { navigator.clipboard.writeText(text); setOk(true); setTimeout(() => setOk(false), 1500); }}
    className="px-2.5 py-1 rounded-lg text-[11px] font-medium bg-[var(--calm-warm)] text-[var(--ink-3)] hover:text-[var(--ink)] hover:bg-[var(--calm-warm)] transition-all border border-black/[0.04] cursor-pointer"
...
export default function OverviewPage() {
  ...
  return (
    <motion.div className="space-y-8" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.23,1,0.32,1] }}>
      <div className="card-calm p-6" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.03)' }}>
        <p className="text-[11px] font-semibold tracking-[0.1em] uppercase text-[var(--ink-3)] mb-3">New API</p>
        <div className="flex gap-3">
          <input value={prompt} onChange={e => setPrompt(e.target.value)} placeholder='Describe your API...' className="input-calm flex-1" onKeyDown={e => { if (e.key === 'Enter') create(); }} />
          <motion.button onClick={create} disabled={!prompt.trim() || creating} className="btn-calm !text-[13px] whitespace-nowrap disabled:opacity-30" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
      ...
      {projects.length > 1 && <div className="flex gap-2 flex-wrap">{projects.map(p => (
        <motion.button key={p.id} onClick={() => setActive(p)} className={`px-4 py-2 rounded-full text-[13px] font-medium cursor-pointer transition-all border
          ${active?.id === p.id ? 'bg-[var(--calm-sage)] text-white border-[var(--calm-sage)]' : 'bg-white text-[var(--ink-2)] border-black/[0.04] hover:border-black/[0.1]'}`}
      ...
      {active && <>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[{ l: 'Base URL', v: baseUrl }, { l: 'API Key', v: active.api_key }].map(x => (
              <div key={x.l}>
                ...
                <div className="flex items-center justify-between bg-[var(--calm-warm)] rounded-xl px-4 py-3 border border-black/[0.04]">
        ...
        <div>
          ...
          <div className="space-y-3">{Object.entries(active.schema_definition).map(([res, fields]) => (
            <motion.div key={res} className="card-calm overflow-hidden" layout>
              <button onClick={() => setExp(p => ({ ...p, [res]: !p[res] }))} className="w-full flex items-center justify-between p-5 hover:bg-[var(--calm-warm)]/50 transition-colors text-left cursor-pointer bg-transparent border-none">
              ...
              <AnimatePresence>{exp[res] && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                  ...
                  <div className="p-5 bg-[var(--calm-warm)]/40">
