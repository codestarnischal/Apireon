...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    ...
    >
      {copied ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5 text-[#6B6B69]" />}
    </button>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-[#6B6B69] mt-1">Test your API endpoints directly in the browser</p>
      </div>

      {/* Request Builder */}
      <div className="card-elevated overflow-hidden">
        {/* Method + URL bar */}
        <div className="flex items-center gap-3 p-4 border-b border-[#E5E4E2]">
          {/* Method selector */}
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-[#FAFAF9] border border-[#E5E4E2] text-[#6B6B69]
            ...
            <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 pointer-events-none text-[#6B6B69]" />
          </div>

          {/* Full URL display */}
          <div className="flex-1 flex items-center gap-2 bg-[#FAFAF9] rounded-lg px-3 py-2.5 border border-[#E5E4E2]">
            <code className="text-sm font-mono text-[#6B6B69] truncate">{fullUrl}</code>
        ...
        <AnimatePresence>
          {['POST', 'PUT'].includes(method) && (
            <motion.div
              ...
              className="border-b border-[#E5E4E2]"
            >
              <div className="flex items-center justify-between px-4 py-2 bg-[#FAFAF9]">
                <span className="text-xs font-semibold text-[#6B6B69] uppercase tracking-wider">Request Body</span>
                <button
                  onClick={() => {
                    const schema = DEMO_SCHEMA[resource];
                    if (schema) setBody(JSON.stringify(generateExamplePayload(schema), null, 2));
                  }}
                  className="text-xs text-[#1A1A19] hover:text-[#1A1A19] transition-colors"
                >
                  Fill example
                </button>
              </div>
              <textarea
                ...
                rows={10}
                spellCheck={false}
                className="w-full bg-transparent px-4 py-3 font-mono text-sm text-[#6B6B69] 
      ...
      <AnimatePresence>
        {response && (
          ...
          >
            {/* Response header */}
            <div className="flex items-center justify-between px-5 py-3 border-b border-[#E5E4E2] bg-[#FAFAF9]">
              ...
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5 text-xs text-[#6B6B69]">
            ...
            {/* Response body */}
            <pre className="p-5 text-sm font-mono text-[#6B6B69] overflow-x-auto max-h-[500px] overflow-y-auto leading-relaxed">
      ...
      <div className="card overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            <Code2 className="h-4 w-4 text-[#1A1A19]" />
            <span className="font-semibold text-white text-sm">Code Snippets</span>
          </div>
          <ChevronDown className={`h-4 w-4 text-[#6B6B69] transition-transform ${showSnippets ? 'rotate-180' : ''}`} />
        </button>
        ...
        <AnimatePresence>
          {showSnippets && (
            <motion.div
              ...
              animate={{ height: 'auto', opacity: 1 }}
              exit={{ height: 0, opacity: 0 }}
              className="border-t border-[#E5E4E2]"
            >
              {/* Language tabs */}
              <div className="flex items-center gap-1 px-4 py-2 bg-[#FAFAF9] border-b border-[#E5E4E2]">
                {(['curl', 'javascript', 'python'] as SnippetLanguage[]).map((lang) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-colors
                               ${snippetLang === lang
                        ? 'bg-[#FAFAF9] text-white'
                        : 'text-[#6B6B69] hover:text-[#6B6B69]'}`}
                  >
                    {lang}
                  </button>
                ))}
              ...
              <pre className="p-5 text-sm font-mono text-[#6B6B69] overflow-x-auto leading-relaxed">
                {getSnippet()}
              </pre>
            </motion.div>
          )}
