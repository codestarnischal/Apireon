...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="p-1.5 rounded hover:bg-[var(--bg-1)]/[0.02] transition-colors"
    >
      {copied ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5 text-[var(--text-2)]" />}
    </button>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-[var(--text-2)] mt-1">Test your API endpoints directly in the browser</p>
      </div>
      ...
      <div className="glass overflow-hidden">
        ...
        <div className="flex items-center gap-3 p-4 border-b border-white/[0.04]">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-[var(--bg-1)]/[0.02] border border-white/[0.04] text-[var(--text-2)]
            ...
            <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 pointer-events-none text-[var(--text-2)]" />
          </div>

          {/* Full URL display */}
          <div className="flex-1 flex items-center gap-2 bg-[var(--bg-1)]/[0.02] rounded-lg px-3 py-2.5 border border-white/[0.04]">
            <code className="text-sm font-mono text-[var(--text-2)] truncate">{fullUrl}</code>
        ...
        <AnimatePresence>
          {['POST', 'PUT'].includes(method) && (
            ...
            >
              <div className="flex items-center justify-between px-4 py-2 bg-[var(--bg-1)]/[0.02]">
                <span className="text-xs font-semibold text-[var(--text-2)] uppercase tracking-wider">Request Body</span>
                <button
                  onClick={() => {
                    const schema = DEMO_SCHEMA[resource];
                    if (schema) setBody(JSON.stringify(generateExamplePayload(schema), null, 2));
              ...
              <textarea
                ...
                rows={10}
                spellCheck={false}
                className="w-full bg-transparent px-4 py-3 font-mono text-sm text-[var(--text-2)] 
      ...
      <AnimatePresence>
        {response && (
          ...
          >
            {/* Response header */}
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.04] bg-[var(--bg-1)]/[0.02]">
            ...
            {/* Response body */}
            <pre className="p-5 text-sm font-mono text-[var(--text-2)] overflow-x-auto max-h-[500px] overflow-y-auto leading-relaxed">
      ...
      <div className="glass overflow-hidden">
        <button
          ...
          className="w-full flex items-center justify-between px-5 py-4 hover:bg-[var(--bg-1)]/[0.02] transition-colors"
        >
          <div className="flex items-center gap-3">
            <Code2 className="h-4 w-4 text-white" />
            <span className="font-semibold text-white text-sm">Code Snippets</span>
          </div>
          <ChevronDown className={`h-4 w-4 text-[var(--text-2)] transition-transform ${showSnippets ? 'rotate-180' : ''}`} />
        </button>
        ...
        <AnimatePresence>
          {showSnippets && (
            ...
            >
              {/* Language tabs */}
              <div className="flex items-center gap-1 px-4 py-2 bg-[var(--bg-1)]/[0.02] border-b border-white/[0.04]">
              ...
              <pre className="p-5 text-sm font-mono text-[var(--text-2)] overflow-x-auto leading-relaxed">
                {getSnippet()}
              </pre>
            </motion.div>
          )}
