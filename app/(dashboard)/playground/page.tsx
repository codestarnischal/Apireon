  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-zinc-400 mt-1">Test your API endpoints directly in the browser</p>
      </div>
        ...
        <div className="flex items-center gap-3 p-4 border-b border-white/[0.06]">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-white/[0.04] border border-white/[0.08] text-zinc-200
            ...
            <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 pointer-events-none text-zinc-500" />
          </div>

          {/* Full URL display */}
          <div className="flex-1 flex items-center gap-2 bg-white/[0.03] rounded-lg px-3 py-2.5 border border-white/[0.05]">
            <code className="text-sm font-mono text-zinc-400 truncate">{fullUrl}</code>
          ...
          {['POST', 'PUT'].includes(method) && (
            ...
            >
              <div className="flex items-center justify-between px-4 py-2 bg-white/[0.02]">
                <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Request Body</span>
                <button
                  onClick={() => {
                    const schema = DEMO_SCHEMA[resource];
                    if (schema) setBody(JSON.stringify(generateExamplePayload(schema), null, 2));
              ...
              <textarea
                ...
                rows={10}
                spellCheck={false}
                className="w-full bg-transparent px-4 py-3 font-mono text-sm text-zinc-300 
      ...
      <AnimatePresence>
        {response && (
          ...
          >
            ...
            {/* Response body */}
            <pre className="p-5 text-sm font-mono text-zinc-300 overflow-x-auto max-h-[500px] overflow-y-auto leading-relaxed">
      ...
      <div className="glass-panel overflow-hidden">
        ...
        >
          ...
          <ChevronDown className={`h-4 w-4 text-zinc-500 transition-transform ${showSnippets ? 'rotate-180' : ''}`} />
        </button>
        ...
        <AnimatePresence>
          {showSnippets && (
            ...
            >
              {/* Language tabs */}
              <div className="flex items-center gap-1 px-4 py-2 bg-white/[0.02] border-b border-white/[0.06]">
