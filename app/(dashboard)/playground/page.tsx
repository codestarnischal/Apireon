...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="p-1.5 rounded hover:bg-[var(--cream)] transition-colors"
    >
...
export default function PlaygroundPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[var(--ink)]">API Playground</h1>
        <p className="text-[var(--ink-2)] mt-1">Test your API endpoints directly in the browser</p>
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="flex items-center gap-3 p-4 border-b border-black/[0.06]">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-[var(--cream)] border border-black/[0.06] text-[var(--ink-2)]
            ...
            <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 pointer-events-none text-[var(--ink-2)]" />
          </div>

          {/* Full URL display */}
          <div className="flex-1 flex items-center gap-2 bg-[var(--cream)] rounded-lg px-3 py-2.5 border border-black/[0.06]">
        ...
        <AnimatePresence>
          {['POST', 'PUT'].includes(method) && (
            ...
            >
              <div className="flex items-center justify-between px-4 py-2 bg-[var(--cream)]">
                ...
                <button
                  ...
                  className="text-xs text-[var(--ink)] hover:text-[var(--ink)] transition-colors"
                >
                  Fill example
                </button>
              </div>
              <textarea
                ...
                rows={10}
                spellCheck={false}
                className="w-full bg-transparent px-4 py-3 font-mono text-sm text-[var(--ink-2)] 
      ...
      <AnimatePresence>
        {response && (
          ...
          >
            {/* Response header */}
            <div className="flex items-center justify-between px-5 py-3 border-b border-black/[0.06] bg-[var(--cream)]">
            ...
            {/* Response body */}
            <pre className="p-5 text-sm font-mono text-[var(--ink-2)] overflow-x-auto max-h-[500px] overflow-y-auto leading-relaxed">
      ...
      <div className="card overflow-hidden">
        <button
          ...
          className="w-full flex items-center justify-between px-5 py-4 hover:bg-[var(--cream)] transition-colors"
        >
          <div className="flex items-center gap-3">
            <Code2 className="h-4 w-4 text-[var(--ink)]" />
            <span className="font-semibold text-[var(--ink)] text-sm">Code Snippets</span>
          </div>
          <ChevronDown className={`h-4 w-4 text-[var(--ink-2)] transition-transform ${showSnippets ? 'rotate-180' : ''}`} />
        </button>
        ...
        <AnimatePresence>
          {showSnippets && (
            ...
            >
              {/* Language tabs */}
              <div className="flex items-center gap-1 px-4 py-2 bg-[var(--cream)] border-b border-black/[0.06]">
              ...
              <pre className="p-5 text-sm font-mono text-[var(--ink-2)] overflow-x-auto leading-relaxed">
                {getSnippet()}
              </pre>
            </motion.div>
          )}
